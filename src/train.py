...
Artifacts written:
  ...
  conv_explainer.pkl (shap.TreeExplainer for conversion model),
  win_model.pkl, win_preprocessor.pkl,
  win_explainer.pkl (shap.TreeExplainer for win model),
...
DATA_PATH = str(_HERE.parent / "data" / "lead_train.csv")
...
CONV_EXPL_OUT  = str(MODELS_DIR / "conv_explainer.pkl")   # SHAP TreeExplainer for conversion model
WIN_MODEL_OUT  = str(MODELS_DIR / "win_model.pkl")
WIN_PRE_OUT    = str(MODELS_DIR / "win_preprocessor.pkl")
WIN_EXPL_OUT   = str(MODELS_DIR / "win_explainer.pkl")    # SHAP TreeExplainer for win model
...
LEAKY_COLS = [
    ...
    "Win_Probability__c",       # latent probability that generated Closed_Won
...
def main() -> None:
    ...
    joblib.dump(conv["explainer"],    CONV_EXPL_OUT)
    joblib.dump(win["model"],         WIN_MODEL_OUT)
    joblib.dump(win["preprocessor"],  WIN_PRE_OUT)
    joblib.dump(win["explainer"],     WIN_EXPL_OUT)
